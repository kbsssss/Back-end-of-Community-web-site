...
이걸 알기에 앞서 우리는 리눅스 커널의 개념과 리눅스에서 센토스와 우분투는 무엇이고
왜 알아야 하는지 그리고 리눅스 버전마다 time_wait이 다르니 EC2에서 리눅스버전과 리눅스커널 버전을
보는 방법에 대해서도 알도록 하겠다. 

> 이왕 얘기가 나왔으니 리눅스의 커널과 센토스 그리고 우분투에 대해 차이점과 그리고 기본적인 내용에
> 대해 조금 정리하고 가도록 하겠다. 
> 그렇다면, 필자가 현재 만들어 놓은 EC2 인스턴스의 경우는 centos 버전이 
