...
로드벨런서로부터 80포트로 요청을 받을때 ec2 보안그룹을 통과하여야 한다. 즉, EC2의 보안그룹에도(빈스톡의 EC2 보안그) 80포트에 대해 
허용을 해주어야지 요청을 받을 수 있는것이다.
...
> 빈스톡 환경 생성을 할때, 보안그룹을 선택(기존에 내가 만들어 놓은 보안그룹) 하던 선택하지 않던
> 디폴트로 생기는 보안그룹이다.
...
이 사진은 빈스톡 환경 구성을 끝내고 다시 인스턴스 카테고리를 편집하여 인스턴스 보안 그룹 설정을 보여주는것이다.
필자가 위에서 필자가 생성한 보안그룹만 체크하고 넘어갔는데, 자동으로 디폴트 보안그룹중 하나가 체크된 모습이다. 이 체크된 디폴트 보안그룹은 두번째 디폴트 보안그룹이다.
...
> 추가로, 굳이 보안그룹ID로 80번 포트에 대해서 요청을 허용하도록 적지않고, HTTP port 80, 0.0.0.0/0로만 보안그룹 목록을
> 추가하면, 이 또한 정상적으로 작동한다.

<br>

> 디폴트 보안그룹은 말 그대로 기본적으로 생기는 것이다. 우리는 ec2 인스턴스를 생성할 때 ssh 접근은 pem키로 접속하여 보안을 강화하고
> 또한, 허용 ip를 내ip로 한다는것을 알고있다.(잘 모르겠다면, ssh와 보안그룹 글을 찾아서 보도록 하자.) 하지만, ec2도 처음 생성할때 보안그룹에서
> ssh가 전체열림으로 되어있고, 이 빈스톡에서도 디폴트 보안그룹에서는 전체열림으로 되어있으나 우리는 이를 보안강화를 위해 내ip로만 접속할 수 있도록 바꾸어 줄 필요가 있다.
> 필자가 위의 작성하여 선택한 보안그룹에는 이미 ssh에 대하여 내 ip로만 접속이 가능하다고 적어놓았으니, 두번째 디폴트 보안그룹에 적혀있는 인바운드 규칙의 ssh에 대해서는
> 삭제하는게 좋다.
...
16.우선, 빈스톡 환경을 생성하자마자 디폴트 보안그룹이 2개 생성이된다.
17.그리고, ssh가 전체열림으로 되어있으니 이거는 빈스톡 환경 구성할때 디폴트로 되어있는값으로 ec2도 보안그룹이 디폴트로 22포트가 전체열림되어있다. 이거를 없애준다.
18.그 그룹ID를 포트번호에 추가하면 ,해당 보안그룹이 사용되어진 곳으로 해당 보안그룹으로 들어온것들이 그대로 내가 보안그룹 설정한 포트번호로 그대로 들어올 수 있다.
19.혹시 ssl적용하기 위해 리스너 추가하면 어떻게 달라지는지 보자.
20.그리고 여기서 포트번호 80을 다른거에서 한번 거치고해서 그룹ID로 적으라고 되어있는데, 로드벨런서는 아닌거같고 그냥 대략적으로 적자.
21.아니야. 기본 디폴트 보안그룹은 맞는데 로드벨런서랑 연결되거나 이런건 아닌거 같아. 그럼 뭐지 ?
22.아냐, 빈스톡 환경생성할때 보안그룹을 선택하지 않아도 기본 22와 80포트있는거만 보안그룹 선택하는거는 똑같다.
23.거기에 내가 그냥 그룹ID로 적혀져있는 80번포트 그거 지우니까 빈스톡 웹 브라우저 배포한거 안들어가짐 그러다가 그냥
80번포트로 ip전체공개로 하니까 들어가졌다.
